<template>
    ...
    <section class="w-11/12 max-w-7xl mx-auto">
        ...
        <section id="seo-allocation" class="mt-10">
            ...
            <p class="mb-4">You may find the most expensive and luxurious {{ $route.params.type }} properties for {{ $route.params.category }} at <span v-html="ListingsStore.listings.data.location.expensive"></span>. While you can find classy yet affordable ones at <span v-html="ListingsStore.listings.data.location.affordable"></span>.</p>
...
export default {  
    ...
    computed: {
        ...
        pageQuestion(){
            return this.$route.params.type.toLowerCase() + ' property for ' + this.$route.params.category + ' in ' + this.titleCase(this.$route.params.area.replace(/-/g, ' ')) +', '+ this.titleCase(this.$route.params.city.replace('-', ' '))
        }, 
        pageIntro(){
            return this.$route.params.type.toLowerCase() + ' properties for ' + this.$route.params.category + ' in ' + this.titleCase(this.$route.params.area.replace(/-/g, ' ')) +', '+ this.titleCase(this.$route.params.city.replace('-', ' '))
        }, 
        pageTitle(){
